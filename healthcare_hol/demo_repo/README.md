...
1. Run the complete setup script in a Snowflake worksheet:
   ...
   @HEALTHCARE_AI_DEMO_REPO/branches/main/healthcare-hol/demo_repo/sql_scripts/demo_setup.sql
   ```

2. The script is pre-configured to use `https://github.com/sfc-gh-pcline/coco-demos.git`. No URL changes are needed unless you have forked the repo, in which case update `API_ALLOWED_PREFIXES` and `ORIGIN` in the script.
